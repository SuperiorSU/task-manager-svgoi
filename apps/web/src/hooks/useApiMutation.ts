 ...
 * toast and a global error toast (via sonner, already mounted in
 * app/providers.tsx) to every mutation, without touching each call site.
 * Error messages always go through getErrorMessage — plain-language
 * ApiError text when the backend sent one, a generic fallback otherwise.
 * Mirrors apps/mobile/src/hooks/useApiMutation.ts. Any `onSuccess`/`onError`
 * passed in still runs — this only adds the toast.
 */
